...
// 后端接口
live2d_settings['modelAPI'] = '/live2d/live2d_api/';   // 自建 API 修改这里
...
function initModel(waifuPath) {
    ...
    $('.waifu-tool .fui-home').click(function () {
		...
		showMessage("诶嘿嘿，调试阶段不会回去哦~",3000);
    });
    ...
    {// waifuWelcome();
        ...
        //     var modelId = 1; // 模型 ID
        //     L = [22, 34, 40, 42, 73]; //73,61
        //     var modelTexturesId = L[Math.floor(Math.random() * L.length - 0.01)];
        ...
        //     // cache: false,
        //     async: true,
        //     success: function (waifuJson) {
        //         $.each(waifuJson.mouseover, function (index, tips) {
        //             $(document).on("mouseover", tips.selector, function () {
        //                 if (tips.selector == ".waifu #live2d" && isHitokoto)
        //                     return;
        //                 var text = tips.text;
...
function loadModel(modelId, modelTexturesId) {
    ...
    // localStorage.setItem('modelTexturesId', modelTexturesId);
    // //loadlive2d('live2d', '//api.fghrsh.net/live2d/rand_textures/?id=' + modelId + '-' + modelTexturesId, console.log('live2d', '模型 ' + modelId + '-' + modelTexturesId + ' 加载完成'));
...
function loadTipsMessage(result) {
    ...
    if (live2d_settings.showF12OpenMsg) {
        if (getChromeVersion() >= 72) {
            ...
            console.log('%c检测控制台是否开启，所用技术参见 https://imjad.cn/archives/lab/check-browser-devtools-open', element);
        } else {
            var re = /x/;
            console.log(re);
    ...
    function showHitokoto() {
        ...
        window.setTimeout(function () { isHitokoto = false; }, 8000);
    }
...
$('.waifu-tool .fui-home').click(function () {
    // window.location = 'http://www.cnblogs.com/TAMING/';
...
{// function waifuWelcome() {
    //     var text;
    //     // var SiteIndexUrl = 'http://www.cnblogs.com/TAMING/'; // 手动指定主页
...
function showHitokoto() {
    ...
    $.getJSON('https://v1.hitokoto.cn/', function (result) {
        isHitokoto = true;
        var text = '这句一言出自 <span style="color:#0099cc;">『{source}』</span>，是 <span style="color:#0099cc;">{creator}</span> 添加的！';
        //var text = '这句一言出处是 <span style="color:#0099cc;">『{source}』</span>，是 <span style="color:#0099cc;">{creator}</span> 在 {date} 收藏的！';
        text = text.render({
            source: result.from,
            creator: result.creator,
            date: "20" + result.created_at.substring(0, 2) + "-" + result.created_at.substring(2, 4) + "-" + result.created_at.substring(4, 6) + " " + result.created_at.substring(6, 8) + ":" + result.created_at.substring(8, 10) + ":" + result.created_at.substring(10, 12)
        });
        showMessage(result.hitokoto, 5000);
        window.setTimeout(function () {
            showMessage(text, 5000);
            setTimeout(function () { isHitokoto = false; }, 5000);
        }, 5000);
    });
}
